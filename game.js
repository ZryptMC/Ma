...
async function boot() {
  ...
  if (localStorage.getItem("dlb_logged_in") === "1") {
    ...
    // TEMP DEBUG: show exactly why the redirect sign-in wasn't picked up,
    // instead of silently falling back to the login screen. Remove this
    // once the Google sign-in issue is confirmed fixed.
    if (window.Multiplayer && Multiplayer.authAvailable()) {
      const dbg = Multiplayer.getLastRedirectDebug();
      showToast("تشخيص: " + dbg);
    }
  }
